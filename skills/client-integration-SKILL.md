...
Daily breakdown across a date range (one dot per UTC day). Includes `totals` across the whole range and zero-fills days with no trades, so the response is always chart-ready.

**Query Parameters:**

| Param | Type | Required | Description |
|-------|------|----------|-------------|
| `from` | string | no | Start date `YYYY-MM-DD` (UTC, inclusive). Default: earliest day with data. |
| `to` | string | no | End date `YYYY-MM-DD` (UTC, inclusive). Default: today. Must be >= `from`. |
| `chain_id` | string | no | Filter by chain. Omit for aggregate across all chains. |
...
| Field | Type | Description |
|-------|------|-------------|
| `from` | string | Effective start date of the range (echo, or earliest day with data). |
| `to` | string | Effective end date of the range (echo, or today UTC). |
| `totals.volume` | number | Sum of USD volume across every day in the range. |
| `totals.trade_count` | number | Total successful trades across the range. |
| `totals.fees` | number | Sum of USD protocol fees across the range. |
| `data_points[].timestamp` | number | Unix seconds at midnight UTC of the day. |
| `data_points[].volume` | number | Total USD volume for the day. `0` if no trades. |
| `data_points[].trade_count` | number | Number of successful trades in the day. `0` if none. |
| `data_points[].fees` | number | Total USD protocol fees for the day. `0` if no trades. |

Read from the pre-aggregated `analytics_daily` rollup. Every day from `from` to `to` inclusive is present (zero-filled for days with no trades). Cache TTL: 60s when range includes today, 1 hour otherwise. Use `/api/v1/analytics/hourly` for per-hour granularity within a single day.

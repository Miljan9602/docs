...
| `total_volume_usd` | number | Total input volume, human-readable units, all time |
| `unique_users` | number | Unique user addresses that have traded |
| `total_fees_collected` | number | Total protocol fees collected, human-readable units |
| `volume_24h` | number | Trading volume in the last 24h |
| `trades_24h` | number | Trades in the last 24h |

Values are in human-readable token units (not wei). Response is Redis-cached for 10 seconds.
...
```json
{
  ...
  "period": "7d",
  "data_points": [
    { "timestamp": 1774243200, "volume": 12345.67, "trade_count": 42 },
    { "timestamp": 1774329600, "volume": 9876.54, "trade_count": 35 }
  ]
}
```
...
| `data_points[].volume` | number | Input volume in this bucket (human-readable units) |
| `data_points[].trade_count` | number | Trades in this bucket |

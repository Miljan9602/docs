...
<ParamField body="intent" type="object" required>
  ...
  <Expandable title="Intent object">
    ...
    <ParamField body="inputAmount" type="string" required>Amount user is selling (wei)</ParamField>
...
curl -X POST "https://solver.example.com/api/v1/intent" \
  ...
  -d '{
    ...
    "intent": {
      ...
      "outputAmount": "985000000000000000"
    }
  }'
```
</RequestExample>
...
<Note>
  The WSS intent request includes additional `fee` and `taker_data` fields not present in the HTTP version. The signing logic is identical. See the [WebSocket Guide](/solver/websocket-guide) for full details.
</Note>

...
<ParamField body="quote_id" type="string" required>
  Coordinator-generated UUID. Echo this back in your response.
</ParamField>
...
<ResponseField name="quote_id" type="string">
  Echo back the coordinator-provided quote_id from the request
</ResponseField>
...
app.post("/api/v1/quote", async (req, res) => {
  ...
  try {
    ...
    res.json(
      successResponse({
        quote_id: req.body.quote_id, // Echo back the coordinator-provided quote_id
        input_token,
        output_token,
        amount_in: amount,
...
<AccordionGroup>
  ...
  <Accordion title="Quote TTL">
    The coordinator sends `expiry_timestamp` based on market configuration. Quote TTL is configurable per market (default 15 seconds). Use the `expiry_timestamp` from the request as your `expiry` in the response.
  </Accordion>

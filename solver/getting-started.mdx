...
Welcome to the RFQ Protocol solver integration guide. Solvers provide liquidity to the protocol by responding to quote requests and executing trades.

## What is a Solver?

A solver is a service that:
...
sequenceDiagram
    ...
    RFQ Coordinator-->>Solver: POST /trade-result (tx_hash)
```
...
| Receive settlement tx_hash | `POST /api/v1/trade-result` | `trade_result` *(no response)* |

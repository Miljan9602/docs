...
sequenceDiagram
    ...
    Note over Coordinator, Solver: After on-chain settlement
    Coordinator->>Solver: trade_result { quote_id, tx_hash }
    Note right of Solver: No response (fire-and-forget)
```
...
<Warning>
  Your solver must be registered for WSS protocol. Contact the team to configure this.
</Warning>
...
| `min_size` | number \| null | no | Minimum order size in input token units |
| `max_size` | number \| null | no | Maximum order size in input token units. Derived from solver on-chain balances. |
...
After on-chain settlement, the coordinator sends a `trade_result` message with the transaction hash. This is a **fire-and-forget** notification — **no response is expected**.
...
| Field | Type | Description |
|-------|------|-------------|
| `quote_id` | string | The original quote ID |
| `tx_hash` | string | Settlement transaction hash (`0x` + 64 hex chars) |

<Note>
  The solver should **not** send a response to this message. Use the `tx_hash` to update your local trade records for analytics and auditing. The transaction may still be pending confirmation when you receive this notification.
</Note>

### Delivery semantics — at-least-once

The coordinator fires `trade_result` from **two independent paths** for every successful settlement:

1. **Sync path** — fired right after the settlement tx is broadcast to the RPC (~immediate). Fire-and-forget; this single attempt may be lost if the solver is unreachable (restarting, network blip, queue overflow).
2. **Indexer backstop** — fired again after the on-chain `RFQSettled` event is confirmed in a reorg-stable block (~10 blocks + ≤5 s indexer tick). Re-tries every indexer tick until the coordinator's `transactions` row is promoted to `success`, so eventual delivery is guaranteed as long as both systems are eventually healthy.

<Warning>
  **Solvers MUST treat the handler as idempotent.** Writing `tx_hash` for the same `quote_id` twice must be a no-op. Do NOT hedge, adjust inventory, or take any non-idempotent action purely on receipt of `trade_result` — verify the receipt on-chain via `getTransactionReceipt` first and gate side effects on a local `tx_status = 'settled'` transition.
</Warning>

Reverted or dropped transactions emit **no events**, so only successful settlements trigger the indexer backstop. A solver that never receives a `trade_result` for a signed intent should independently verify the tx status (the coordinator's late-reconcile sweep also marks stale `submitted` rows as failed, but that is not pushed to the solver).
...
**Request:**
```bash
curl -X GET "https://rfq.saphyre.xyz/api/v1/config?chain_id=1329"
```
  ...
  ws.on("message", (raw) => {
    ...
    switch (msg.action) {
      ...
      case "trade_result":
        ...
        await saveTradeResult(quote_id, tx_hash); // your storage logic
        break;
    }
  });
    ...
    while let Some(Ok(Message::Binary(data))) = rx.next().await {
        ...
        match msg["action"].as_str().unwrap_or("") {
            ...
            "trade_result" => {
                ...
                save_trade_result(quote_id, tx_hash); // your storage logic
            }
            _ => {}
        }
    }

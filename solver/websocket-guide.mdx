...
| `taker_data.swap_intent` | object | Taker's original swap intent (amounts as signed by the user, for on-chain verification) |
| `taker_data.swap_intent.inputAmount` | string | Taker's original input amount (the amount the user actually signed) |
| `taker_data.signature_params` | object | Taker's Permit2 signature parameters |

<Note>
  Both WSS and HTTP transports use the same **nested** `taker_data` structure with `taker_data.swap_intent` and `taker_data.signature_params`. The `swap_intent.inputAmount` is the original amount the user signed, while `intent.inputAmount` is the net amount the solver should use for intent reversal.
</Note>

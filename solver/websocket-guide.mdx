...
```
wss://dev-rfq.saphyre.xyz/api/v2/solver-stream
```
...
| `intent.inputAmount` | string | Taker's input amount in wei (post-fee: original amount minus protocol fee). Use this as the maker's `outputAmount`. |
| `intent.outputAmount` | string | Taker's output amount in wei |
| `taker_data.swap_intent` | object | Taker's original swap intent (pre-fee amounts, for on-chain verification) |
| `taker_data.swap_intent.inputAmount` | string | Taker's **pre-fee** input amount (the amount the user actually signed) |
| `taker_data.signature_params` | object | Taker's Permit2 signature parameters |

<Note>
  Both WSS and HTTP transports use the same **nested** `taker_data` structure with `taker_data.swap_intent` and `taker_data.signature_params`. The `swap_intent.inputAmount` is the **pre-fee** amount the user signed, while `intent.inputAmount` is the **post-fee** amount (original minus protocol fee).
</Note>
...
RFQ_SERVER_WS_URL=wss://dev-rfq.saphyre.xyz/api/v2/solver-stream
...
const WS_URL = "wss://dev-rfq.saphyre.xyz/api/v2/solver-stream";
...
async fn main() -> anyhow::Result<()> {
    let url = "wss://dev-rfq.saphyre.xyz/api/v2/solver-stream";
    ...
    while let Some(Ok(Message::Binary(data))) = rx.next().await {
        ...
        match msg["action"].as_str().unwrap_or("") {
            ...
            "ping" => {
                tx.send(Message::Binary(to_vec(&json!({ "action": "pong" }))?))
                    .await?;
            }
            "get_markets" => {
                let resp = json!({

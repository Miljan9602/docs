...
**Key fields:**
- `intent.inputAmount` is the **post-fee** amount (original user amount minus protocol fee). This is what the solver should use as the maker's `outputAmount`.
- `taker_data.swap_intent.inputAmount` is the **original (pre-fee)** amount that the user signed.
- `fee` is the protocol fee in input token wei.

...
- `taker_data.swap_intent.inputAmount` is the original amount that the user signed.

<Note>
  Both HTTP and WSS transports use the same **nested** `taker_data` structure with `taker_data.swap_intent` and `taker_data.signature_params`. See the [WebSocket Guide](/solver/websocket-guide) for WSS format details.
</Note>

...
The RFQSettlement contract has minimal mutability. The contract owner can:

### `setFeeTreasury(address)`

Allows the owner to update the fee treasury address. Only callable by the contract owner.
...
Allows recovering tokens accidentally sent to the contract. This is a safety function -- during normal operation, tokens flow through Permit2 and are never held by the contract.
...
<Warning>
  The `rescueTokens` function is NOT restricted to owner-only. Any address can call it to sweep tokens held by the contract.
</Warning>

<Note>
  All other contract parameters (Permit2 address, WETH9 address) are immutable and set at deployment. Always verify the contract address via the config endpoint before interacting.
</Note>

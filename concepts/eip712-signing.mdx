...
// 1. Get config
const configResponse = await fetch('https://rfq.saphyre.xyz/api/v1/config?chain_id=1329');
const config = await configResponse.json();
const PERMIT2 = config.permit2_address;
const RFQ_SETTLEMENT = config.rfq_settlement_address;

// 2. Get quote
const quote = await fetch('https://rfq.saphyre.xyz/api/v1/quote?...').then(r => r.json());
...
// 5. Submit intent
const result = await fetch('https://rfq.saphyre.xyz/api/v1/intent', {
  method: 'POST',

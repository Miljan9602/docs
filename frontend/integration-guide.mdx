...
<AccordionGroup>
  ...
  <Accordion title="Validate Input Amounts">
    Compare the user's input amount against `max_size` from the directional market entry (A→B has its own `max_size` in input token units). If the amount exceeds `max_size`, skip the quote request and show a validation error to the user. This prevents unnecessary quote requests for amounts that exceed available solver liquidity.
  </Accordion>

...
<ParamField body="swap_intent" type="object" required>
  ...
  <Expandable title="SwapIntent object">
    ...
    <ParamField body="unwrap" type="boolean" required>
      Set to `true` only when the output token is the chain's native wrapped token (WSEI/WETH) to receive native currency instead. Set to `false` for all other cases.
    </ParamField>
...
response = requests.post(
    "https://dev-rfq.saphyre.xyz/api/v1/intent",
    json=intent
)
data = response.json()
...
```json 409 - Quote Already Processing
{
  "status": "error",
  "error": "Quote is already being processed",
  "code": "QUOTE_ALREADY_PROCESSING"
}
```

```json 400 - Quote Rejected
{
  "status": "error",
...
- The `deadline` in `signature_params` must be a future Unix timestamp. Expired deadlines will be rejected
- Use a random 256-bit integer for `nonce` (Permit2 uses unordered nonces)
- The `inputToken` and `outputToken` in your `swap_intent` must exactly match the tokens from the original quote
